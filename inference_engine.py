...
class IntegratedEngine:
    ...
    def analyze_anomalib(self, image: Image.Image) -> dict[str, Any]:
        ...
        return {"heatmap": heatmap_resized, "score": float(pred_score), "peak_point": (int(peak_x), int(peak_y))}
    ...
    def create_overlay(
        self, image: Image.Image, mask: np.ndarray, color: tuple[int, int, int] = (255, 0, 0), alpha: float = 0.5
    ) -> Image.Image:
